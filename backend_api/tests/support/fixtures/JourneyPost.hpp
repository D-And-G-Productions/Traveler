...
public:
  cpr::Response journeyPost(const string &token, const JourneyCreateRequest &journeyCreateRequest) {
  ...
  Journey getJourney(int64_t journeyId) { return testServer->journeyRepo->selectById(journeyId); }
};
